...
interface PostRepository : CrudRepository<Post, Int> {
    ...
    @Query(value = "select from post where author=(select from user where latitude>=:lat-10.0 and " +
            "latitude<=:lat+10.0 and longitude>=:lng-10.0 and longitude<=:lng+10.0) order by added_millis desc", nativeQuery = true)
