...
interface PostRepository : CrudRepository<Post, Int> {
    ...
    @Query(value = "select from post where author=(select from user where latitude>=:lat-1.0 and " +
            "latitude<=:lat+1.0 and longitude>=:lng-1.0 and longitude<=:lng+1.0) order by added_millis desc", nativeQuery = true)
